...
Two source shapes are supported, both keyed by an opaque session key:

- a finished file on disk (download playback), read by FFmpeg directly;
- a byte stream the caller supplies (Converted preview of a provider URL),
  fed to FFmpeg over stdin.

The stream shape exists so provider URLs — which embed account credentials —
never reach FFmpeg's argv, where `ps` would expose them to every user on the
host. The caller opens the provider connection; FFmpeg only ever sees
`pipe:0`.
"""
